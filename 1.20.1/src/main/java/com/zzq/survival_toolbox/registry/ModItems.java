...
public class ModItems {
    ...
    public static final RegistryObject<Item> CAPTURED_ENTITY =
            ITEMS.register("captured_entity", CapturedEntityItem::new);

    /** 除你装备（卸除怪物装备的法杖） */
    public static final RegistryObject<Item> DISARM_STAFF =
            ITEMS.register("disarm_staff", DisarmStaffItem::new);

    /** 地形编辑器（多功能范围操作工具） */
    public static final RegistryObject<Item> TERRAIN_EDITOR =
            ITEMS.register("terrain_editor", TerrainEditorItem::new);

    /** 静态拴绳（生物拴绳增强版，支持AI冻结与栅栏绑定） */
    public static final RegistryObject<Item> STATIC_LEASH =
            ITEMS.register("static_leash", StaticLeashItem::new);

    /** 黑白名单物品（用于伤害过滤与实体记录） */
    public static final RegistryObject<Item> BLACKLIST =
            ITEMS.register("blacklist", BlacklistItem::new);

    /** 附魔包（一次性生成两本自定义附魔书） */
    public static final RegistryObject<Item> ENCHANTMENT_BAG =
            ITEMS.register("enchantment_bag", EnchantmentBagItem::new);

    /** 透视眼镜（手持时透视白名单中的方块） */
    public static final RegistryObject<Item> XRAY_GOGGLES =
