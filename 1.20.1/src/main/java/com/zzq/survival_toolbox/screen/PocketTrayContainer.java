 ...
 * （左键整叠、右键放1/取半、Shift 转、拖拽分配、双击收集全部由原版实现且行为标准）。
 * 需要单独接管的只有两类：
 * <ul>
 ...
 * （1.20.1 原版同步用 byte 写数量，>127 会被截断，不能用来传 mB）。
 * </p>
 */
public class PocketTrayContainer implements Container {
    ...
    public PocketTrayContainer(ItemStack bag, boolean clientSide) {
        this.bag = bag;
     ...
     * <p>
     * 为什么流体格要显式同步：原版槽位同步本来该把这件"桶"发下来，但实测
    ...
    /** 该格能不能放这种流体（空格或同种流体） */
    public boolean canAcceptFluid(int slot, net.minecraftforge.fluids.FluidStack stack) {
     ...
     */
    public long insertFluid(int slot, net.minecraftforge.fluids.FluidStack stack) {
     ...
     */
    public net.minecraftforge.fluids.FluidStack extractFluid(int slot, long maxAmount) {
        PocketStorageHelper.FluidEntry entry = fluidAt(slot);
        if (entry == null || maxAmount <= 0) return net.minecraftforge.fluids.FluidStack.EMPTY;
        ...
        if (take <= 0) return net.minecraftforge.fluids.FluidStack.EMPTY;
        net.minecraftforge.fluids.FluidStack outStack = entry.toStack(take);
