...
import com.zzq.survival_toolbox.network.PocketDimensionSyncPacket;
...
public class PocketDimensionMenu extends AbstractContainerMenu {
    ...
    /** 某个功能页展开时，面板总共要占多高（够放内容即可，不要撑满整窗） */
    ...
    public static final int STONE_RESULT = STONE_IN_BASE + 1;
    /** 功能页槽位的总长度（含所有页） */
    ...
    /** 熔炼页：左边一列"输入 → 火 → 燃料"，右边"→ 产物预览"（产物直接进储物，这里只显示会烧出什么） */
    ...
    /** 磁铁页：9 格名单（3×3）+ 两行按钮（开关/模式、范围 −＋/黑白名单） */
    ...
    /** 切石机页：左边输入格、右边产物格，再往右是"可用配方"列表（3 列 × 6 行，滚轮翻页） */
    public static final int STONE_INPUT_X = PAGE_PANEL_X + 8;
    public static final int STONE_INPUT_Y = 72;   // ⚠️ 必须在列表区(x=+32..+116)左侧，且不要和标题/按钮重叠
    ...
    /** 选中的切割配方下标（在"按 id 排序的可用配方"列表里；-1 = 没有可用配方） */
    ...
    private final List<PocketDimensionSyncPacket.SlotData> lastSyncedSlots = new java.util.ArrayList<>();
    ...
    private List<ItemStack> lastSyncedTrayIcons = new java.util.ArrayList<>(
            java.util.Collections.nCopies(PocketTrayStorage.SLOTS, ItemStack.EMPTY));
     ...
     * 这里自己记录"是否变化"，只在确实变化时才让原版发一次包，不会每 tick 刷包。
     * </p>
     */
     ...
     * 下次打开时输入槽是空的——若不区分，上次留下的预览就会被当成玩家自己摆放的材料而凭空获得。
     * </p>
     */
    ...
    /**
     * 合成页的"自动补充"开关（同步给客户端显示；照精妙背包的做法加这么一个按钮）。
     * <p>
    ...
    public PocketDimensionMenu(int windowId, Inventory playerInv, ItemStack bag) {
        ...
        if (!clientSide) {
            this.furnaceToSlot = PocketFurnace.read(bag).toSlot;
        }
        this.container = new PocketDimensionContainer(bag, playerInv.player);
        this.tray = new PocketTrayContainer(bag, clientSide);
        this.furnace = new PocketFurnaceContainer(bag, playerInv.player.level(), clientSide);
        this.anvilIn = new PocketPageContainer(bag, "AnvilIn", 2, clientSide);
        this.anvilOut = new PocketResultContainer();
        this.smithIn = new PocketPageContainer(bag, "SmithIn", 3, clientSide);
        this.smithOut = new PocketResultContainer();
        this.craftIn = new PocketPageContainer(bag, "CraftIn", 9, clientSide);
        this.craftOut = new PocketResultContainer();
        this.disIn = new PocketPageContainer(bag, "DisIn", 1, clientSide);
        this.disMat = new PocketPageContainer(bag, "DisMat", 9, clientSide);
        this.disOut = new PocketResultContainer();
        this.stoneIn = new PocketPageContainer(bag, "StoneIn", 1, clientSide);
        this.stoneOut = new PocketResultContainer();
        this.magFilter = new PocketFilterContainer(bag, com.zzq.survival_toolbox.util.PocketMagnet.TAG_FILTER,
        ...
        if (!clientSide) {
            ...
            this.craftRefillSlot.set(com.zzq.survival_toolbox.util.PocketCraftRefill.read(bag) ? 1 : 0);
        }
        this.addDataSlot(this.disIndexSlot);
        this.addDataSlot(this.disTotalSlot);
        if (!clientSide) {
            net.minecraft.nbt.CompoundTag root = bag.getTag();
        ...
        // 熔炉页两个格（输入、燃料；面板收起时同样 isActive()=false 不显示也点不到）
    ...
    private void refreshMagnet() {
        ...
        com.zzq.survival_toolbox.util.PocketMagnet.State st =
                com.zzq.survival_toolbox.util.PocketMagnet.read(bag);
    ...
    private void refreshRestock() {
        ...
        com.zzq.survival_toolbox.util.PocketRestock.State st =
                com.zzq.survival_toolbox.util.PocketRestock.read(bag);
    ...
    private void updateRestock(java.util.function.Consumer<com.zzq.survival_toolbox.util.PocketRestock.State> action) {
        com.zzq.survival_toolbox.util.PocketRestock.State st =
                com.zzq.survival_toolbox.util.PocketRestock.read(bag);
        action.accept(st);
        com.zzq.survival_toolbox.util.PocketRestock.write(bag, st);
        refreshRestock();
    }
    ...
    private void refreshCraftRefill() {
        if (player.level().isClientSide) return;
        this.craftRefillSlot.set(
                com.zzq.survival_toolbox.util.PocketCraftRefill.read(bag) ? 1 : 0);
    }

    /** 服务端：翻转"自动补充"开关并写回袋子 NBT（客户端只发动作包，状态服务端权威） */
    private void toggleCraftRefill() {
        if (player.level().isClientSide) return;
        boolean on = !com.zzq.survival_toolbox.util.PocketCraftRefill.read(bag);
        com.zzq.survival_toolbox.util.PocketCraftRefill.write(bag, on);
        refreshCraftRefill();
    }
    ...
    private void updateMagnet(java.util.function.Consumer<com.zzq.survival_toolbox.util.PocketMagnet.State> action) {
        com.zzq.survival_toolbox.util.PocketMagnet.State st =
                com.zzq.survival_toolbox.util.PocketMagnet.read(bag);
        action.accept(st);
        com.zzq.survival_toolbox.util.PocketMagnet.write(bag, st);
        refreshMagnet();
    }
     ...
     * 下一次保存会被旧缓存整份覆盖 —— 表现就是"东西放进去就没了"（实测）。
     * </p>
     *
     * @return 剩余数量（恒为 0）
     */
     ...
     * 表现就是"材料取出来了、袋子里一个没少"，也就是 JEI 的 + <b>凭空复制材料</b>（实测）。
     * </p>
     *
     ...
     * <p>
     * 规则：<b>打开哪个就匹配哪个</b>。搜索视图是例外的——它本来就是"把所有页的结果摊开看"，
     ...
     */
    public long depositFluidToStorage(net.minecraftforge.fluids.FluidStack stack) {
     ...
     * 表现就是"东西被吸进去了，打开袋子什么都没有"（实测的丢物品表现）。
     * </p>
     */
    ...
    public static long depositToOpenMenu(net.minecraft.server.level.ServerPlayer player,
        ...
        return PocketStorageHelper.quickDeposit(player, bag, stack);
    }
    ...
    /** 服务端：惰性创建原版引擎（铁砧 / 锻造台菜单对象，不注册给玩家，只用来算） */
     ...
     * <p>
     * ⚠️ 顺序必须和客户端算的一模一样（客户端也是"筛掉不匹配的 + 按 id 排序"），
     * 这样选中项只传**下标**就不会错位；万一下标对不上，这里还会用 {@code test(input)} 再确认一次，
     * 宁可没产物也不给错东西。
     * </p>
     */
    private List<net.minecraft.world.item.crafting.StonecutterRecipe> stonecutterCandidates(ItemStack input) {
        List<net.minecraft.world.item.crafting.StonecutterRecipe> out = new java.util.ArrayList<>();
        ...
        out.sort(java.util.Comparator.comparing(r -> r.getId().toString()));
        return out;
    }
    ...
    /** 按"输入 + 选中配方"重算产物格（输入变了 / 打开页 / 取走一次之后都要调） */
    private void refreshStonecutter() {
        if (player.level().isClientSide) return;
        List<net.minecraft.world.item.crafting.StonecutterRecipe> list = stonecutterCandidates(stoneIn.getItem(0));
        ...
        stoneOut.setResult(list.isEmpty()
                ? ItemStack.EMPTY
                : list.get(stoneRecipeIndex).getResultItem(player.level().registryAccess()).copy());
    }

    /**
     * 取走切石机页的产物：按原版切石机规则，取一次消耗 1 个输入
     * （产物本身照旧是"光标拿着 / Shift 整叠进袋子"）。
     */
    private void clickedStonecutterResult(ClickType clickType, Player player) {
        ...
        stoneIn.setChanged();
        refreshStonecutter();                 // 输入少了一个：产物按原版重新算
    }
    ...
    private void refreshDisassemble() {
        ...
        if (!preview) {
            // 合成模式：把玩家摆的材料喂进去（只有确实变了才喂，避免无谓地重算一遍配方）
    ...
    private void mirrorDisassemble(com.zzq.survival_toolbox.util.PocketDisassembleEngine e, boolean preview) {
        // ⚠️ 页格子的 setItem 会立刻把内容写进袋子 NBT，所以"内容没变就不要写"：
     ...
     * <p>
     * 见 {@link #disPreview} 的说明：不记的话，"预览留下、输入被拿走"会让预览凭空变成玩家的材料。
     * 只有变化时才写 NBT，避免每 tick 都动一次袋子数据（会引发布袋每 tick 重新同步）。
     * </p>
     */
     ...
     * 花费算 0 → 产物直接是空 —— 表现就是"输入名字后，改名后的武器拿不出来"（实测）。
     * </p>
     ...
     * <p>
     * 只有内容确实变了才喂（原版 {@code createResult} 每次都要算附魔/耐久，不要每 tick 重复计算）；
    ...
    private void refreshPageEngine(int panel) {
        ...
        try {
            if (panel == PANEL_ANVIL) {
                ...
                //    以前是"关掉铁砧页就清空 anvilName"，副作用是玩家刚打完字、误点了页按钮，输入就被丢弃；
            ...
            com.mojang.logging.LogUtils.getLogger()
                    .error("[次元袋] 铁砧/锻造台/合成页的原版引擎刷新失败，已停用这几页（详见堆栈）", t);
        }
    }
     ...
     * 取走铁砧/锻造台的产物。
     * <p>
     * <b>能不能取、取了扣什么，全部问原版</b>：能取 = 原版结果格的 {@code mayPickup}
    ...
    private void clickedPageResult(int slotId, ClickType clickType, Player player) {
        ...
        if (clickType == ClickType.QUICK_MOVE) {
            // ⚠️ 规则："工作台方块那种 Shift 批量拿产物，袋子里也要"。
            //    原版工作台的 Shift+左键是**一直做到材料用完**（反复取产物），这里以前只做一次，
            //    所以袋子里只能一个个拿。上限是防呆：万一某个配方取走产物却不消耗材料，不至于把服务端卡死。
            int made = 0;
     ...
     * @return 是否真的取走了一次（false = 产物为空 / 不能取 / 袋子收不下）
     */
    private boolean takePageResultOnce(int panel, int kind, PocketPageContainer inputs, boolean anvil,
        ...
        try {
            ...
            e.take(taken);                       // 原版 onTake
            pullFromEngine(e, inputs);
            if (anvil) {
                anvilName = "";                  // 原版取完工品会清掉改名，这里跟着清
                e.setItemName("");
            }
     ...
     * 取走拆解页的产物格。
     * <p>
     * 两种模式都<b>只问原版（拆解台自己那套代码）</b>：
     * <ul>
     *   <li>拆解模式（输入槽有东西）：产物格显示的是"要被拆的那件东西"，取走 =
    ...
    private void clickedDisassembleResult(ClickType clickType, Player player) {
        ...
        try {
            ...
            //    老写法是"先取一份副本、调 onTake、再看产物格空没空"——合成成功但九宫格还有剩料时，
            //    原版会把产物重新画回产物格当下一条的预览，于是"格空了"永远不成立：
            //    材料被吃掉、产物却拿不到；只有"最后一次把材料用光"时才碰巧能拿到
            //    （实测："一直点拿不到产物 / 光消耗拿不到 / 最后一次能拿出来"）。
            ...
            if (!e.lastSettleSucceeded()) {
                // 结算没成（材料在预览后被动过之类）：产物放回去，绝不让玩家凭空获得物品
            ...
            // 直接给玩家了，这一份绝不能也发出去（否则等于玩家凭空获得一件输入物品）
            mirrorDisassemble(e, !disIn.getItem(0).isEmpty());
        } catch (Throwable t) {
            pageEngineBroken = true;
            com.mojang.logging.LogUtils.getLogger()
     ...
     * 磁铁页名单格（幽灵条目）。
     * <p>
     * 规则：往名单里放东西 = <b>复制一份物品信息</b>，手里的实物<b>退回袋子储物空间</b>；
    ...
    private void clickedMagnetFilter(int index, int button, ClickType clickType, Player player) {
        ...
        if (button == 1) {
            this.magFilter.removeSample(index);
            this.entriesDirty = true;
            return;
        }
        ItemStack cursor = getCarried();
        if (cursor.isEmpty()) return;
        this.magFilter.addSample(cursor);
        ItemStack hand = cursor.copy();
        setCarried(ItemStack.EMPTY);
        long left = depositToStorage(hand);
        if (left > 0) setCarried(cursor);
        this.entriesDirty = true;
    }
    ...
    public void handleTrayAction(int action) {
        if (action == com.zzq.survival_toolbox.network.PocketTrayActionPacket.ACTION_OPEN) {
            ...
            if (this.disPreview) {
                ...
                // 直接丢掉（输入还在的话原版会重新算出来），避免被当成玩家自己摆放的材料而凭空获得
            ...
            // 否则玩家刚打完字就点了页按钮，输入的内容会被丢弃（实测"改名不生效"）。
    ...
    public void setSyncedData(int pageOffset, int currentPage, List<String> pageNames,
                              ...
                              List<PocketDimensionSyncPacket.SlotData> slots,
        ...
        // 托盘流体格的"桶图标/流体本体"由同步包补齐（不要依赖原版槽位同步，见 PocketTrayContainer#fluidIcons）
     ...
     * <p>
     * 规则：只做本地 → 共享，而且**不覆盖**共享里已有的东西（两边相加）；
     ...
     * <p>
     * ⚠️ 光标必须算进来（曾修复的 bug）：以前只看双手，于是"拿着空桶（在光标上）左键点流体格"
    ...
    private boolean canAcceptInto(HeldFluid held, int slotId) {
        FluidStack inBag = container.peekFluidAt(slotId);
        if (inBag.isEmpty()) return false;
        int capacity = held.handler().getTankCapacity(0) - held.handler().getFluidInTank(0).getAmount();
        if (capacity <= 0) return false;
        FluidStack inContainer = held.handler().getFluidInTank(0);
        return inContainer.isEmpty() || inContainer.isFluidEqual(inBag);
    }
    ...
    private void fillContainerFromSlot(int slotId, HeldFluid held, Player player) {
        ...
        FluidStack taken = container.extractFluidAt(slotId, capacity);
        if (taken.isEmpty()) return;
        int accepted = held.handler().fill(taken, IFluidHandler.FluidAction.EXECUTE);
        if (accepted <= 0) {
            // 容器实际上装不下（例如这种流体没有对应的桶）：原样放回这一格
        ...
        writeHeld(player, held.hand(), held.handler().getContainer());
        this.entriesDirty = true;
    }
     ...
     * （实测："不要合并，放哪是哪"）。合并只应该发生在"整理"和"手动倒进同一格"时。
     * </p>
     */
    private void pourContainerIntoSlot(int slotId, HeldFluid held, Player player) {
        ...
        if (left > 0) {
            // 这一格放不下（被物品占着 / 是另一种流体）：倒回容器，绝不擅自塞到别处
    ...
    private void clickedTrayFluid(int traySlot, int button, Player player) {
        if (button == 0) {
            ...
            // 托盘按原版箱子处理（每格一叠、≤64），整叠放不下就不换，不要把多余的部分吃掉
     ...
     *   <li>玩家背包格、界面外：不落地，按当前模式收回袋子存储（物品始终没有离开袋子内部）</li>
     * </ul>
     * <p>
     * ⚠️ 交换这条是后来补上的规则："拿东西左键点流体格能换位置，拿着流体左键点物品格也要能换位置" ——
    ...
    private void dropCarriedFluid(int slotId, PocketStorageHelper.FluidEntry carried, Player player) {
        ...
        if (slotId >= TRAY_BASE && slotId < TRAY_BASE + PocketTrayStorage.SLOTS) {
            ...
            if (left == want) {
                ...
                ItemStack inTray = tray.getItem(traySlot);
                int max = Math.min(inTray.getMaxStackSize(), tray.getMaxStackSize());
                if (!tray.isFluidAt(traySlot) && !inTray.isEmpty() && inTray.getCount() <= max) {
                    ItemStack taken = tray.removeItem(traySlot, inTray.getCount());
            ...
            if (left == want) {
                // 同理：物品占着这一格时，能整格拿起来就交换（规则："拿着流体左键点物品格也能换位置"）
            ...
            // 玩家背包格 / 界面外：不落地，按当前模式收回袋子存储（物品始终没有离开袋子内部）
            depositFluidToStorage(player, carried);
            setCarried(ItemStack.EMPTY);
            this.entriesDirty = true;
            return;
    ...
    private void writeHeld(Player player, int hand, ItemStack after) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index >= FURNACE_BASE) {
            ...
            //   拖进页里的，Shift 一下就原路收回储物空间，比塞进身上背包更符合习惯；
        ...
        if (index >= TRAY_BASE) {
            // 托盘 Shift+左键 = 把这一叠收回来，**优先收回储物空间**（规则：
            // "托盘 shift 左键不是优先回袋子、而是进入物品栏"）。
            // 储物空间不收的那部分（例如袋子这类被拉黑的东西）再退回老行为：塞玩家背包，满了就留在托盘。
            ...
            // ② 剩下这些储物空间不收 → 老行为：塞玩家背包（这里必须用格内"活堆叠"，
        ...
        if (index < PocketDimensionContainer.PAGE_SIZE) {
            // 次元袋格 → Shift+左键：**面板开着就先放进该功能栏**（规则：
            // "袋子里的物品按 Shift 左键也要优先进旁边的功能里"，且"**托盘开着也要先进托盘**"），
            // 放不进再按老规矩取一组到背包。
            if (panel != PANEL_NONE) {
        ...
        // ① 侧边功能页：**面板展开时优先放进该功能栏**（拆解台页优先放九宫格）。
        //    托盘不在这一支里，它有自己的分支（下面 ②）。
        if (panel != PANEL_NONE && panel != PANEL_TRAY) {
            ...
            // 一件都没放进去（输入格不匹配/只读）：继续按老路子落进储物空间，不要让 Shift+左键没反应
        }
     ...
     * 拆解台页按既定规则<b>先九宫格、再左边输入格</b>：拆解模式下九宫格是只读预览（放不进去），
    ...
    private int moveIntoOpenPanel(ItemStack stack) {
        ...
        switch (panel) {
            ...
            case PANEL_DISASSEMBLE -> {
                ...
                //    —— 一部分还给玩家（表现为"一个进了背包"）、一部分被清掉（表现为"还有一个不翼而飞"）。
                //    规则是"shift 左键只给拆解台的输入格一个"，所以就放一个到输入格、到此为止。
                ...
                if (!there.isEmpty() && !PocketStorageHelper.sameItem(there, stack)) return 0;   // 输入格已有别的东西：不强行放入
                if (there.isEmpty()) {
            ...
            case PANEL_TRAY -> {
                // 托盘（规则："托盘也要打开时在袋子里 Shift 左键优先放托盘里"）。
                // ⚠️ 必须**跳过流体格**：那种格子里的"桶"只是显示图标，把物品放进去会和流体打架
                //   （旧版本曾出现"物品压上去让流体隐形、整理时被清掉"）。
     ...
     * <p>
     * 先按老逻辑"插进当前页的空位"（{@link PocketDimensionContainer#insert}，现在会避开流体格）；
    ...
    public void clicked(int slotId, int button, ClickType clickType, Player player) {
        ...
        if (inStorage && button == 1 && clickType == ClickType.PICKUP && !container.isSearching()) {
            ...
            boolean cursorFree = carried.isEmpty()
                    || net.minecraftforge.fluids.FluidUtil.getFluidHandler(carried).isPresent();
            ...
            if (button == 0) {
                if (cursor.isEmpty()) {
                    ...
                    if (inSlot.isEmpty() || PocketStorageHelper.sameItem(cursor, inSlot)) {
                        ...
                        // ⚠️ 老写法是"只取一组 + setItemAt 覆盖整格"，大堆叠的剩余部分会被**吃掉**（已修复）。
    ...
    public void broadcastChanges() {
        ...
        // 条目/翻页/过滤/页操作/托盘/熔炉变化时同步显示数据给客户端
        List<PocketDimensionSyncPacket.SlotData> slots = container.getSyncSlots();
        ...
        if (entriesDirty
            ...
            if (player instanceof net.minecraft.server.level.ServerPlayer sp) {
                SurvivalToolbox.CHANNEL.send(PacketDistributor.PLAYER.with(() -> sp),
                        new PocketDimensionSyncPacket(container.getPageOffset(), container.getCurrentPage(),
    ...
    /** 托盘流体格的显示栈有没有变（变了才重发同步包；对比物品 + NBT） */
    ...
    private static boolean sameSlots(List<PocketDimensionSyncPacket.SlotData> a,
                                     List<PocketDimensionSyncPacket.SlotData> b) {
        if (a.size() != b.size()) return false;
        for (int i = 0; i < a.size(); i++) {
            PocketDimensionSyncPacket.SlotData x = a.get(i);
            PocketDimensionSyncPacket.SlotData y = b.get(i);
