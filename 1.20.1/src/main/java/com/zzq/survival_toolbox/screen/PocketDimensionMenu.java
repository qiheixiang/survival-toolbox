...
public class PocketDimensionMenu extends AbstractContainerMenu {
    ...
    public PocketDimensionMenu(int windowId, Inventory playerInv, ItemStack bag) {
        ...
        this.bag = bag;
        this.container = new PocketDimensionContainer(bag);
    ...
    public void clicked(int slotId, int button, net.minecraft.world.inventory.ClickType clickType, Player player) {
        ...
        // 拖拽放置（QUICK_CRAFT）：把物品放到最后拖到的存储格（默认逻辑会塞到自动位置）
        if (clickType == net.minecraft.world.inventory.ClickType.QUICK_CRAFT) {
            if (slotId >= 0 && slotId < PocketDimensionContainer.PAGE_SIZE) {
                lastDragSlot = slotId;
                return;
            }
            if (slotId == -999 && button == 4) {
                lastDragSlot = -1;
                return;
            }
            if (slotId == -999 && button == 6) {
                if (lastDragSlot >= 0) {
                    ItemStack cursor = getCarried();
                    if (!cursor.isEmpty() && !container.isSearching()) {
                        container.setItemAt(lastDragSlot, cursor);
