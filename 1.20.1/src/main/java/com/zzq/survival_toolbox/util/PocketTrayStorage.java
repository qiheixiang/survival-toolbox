 ...
 * </p>
 * <p>
 * <b>只存在袋子自己的 NBT 里</b>（与 {@code Pages}/{@code FluidPages} 同一个根标签，
 ...
 * </p>
 * <p>
 * 与 1.21.1 的差异（1.20.1 没有数据组件，也没有 ItemNbt 门面）：
 * 物品 NBT 直接走 {@code stack.getTag()}/{@code stack.save(tag)}；
 * 写物品时必须先写好 {@code Count}——1.20.1 的 {@code ItemStack} 构造用
 * {@code getByte("Count")} 读数量，缺键或为 0 会被判成空物品而丢失。
 * </p>
 */
public final class PocketTrayStorage {
    ...
    public static Tray read(ItemStack bag) {
        CompoundTag root = bag.getTag();
        return root == null ? new Tray() : read(root);
    }

    /** 从一段自定义数据根标签读取托盘 */
    public static Tray read(CompoundTag root) {
        Tray tray = new Tray();
        ...
        if (root.contains(TAG_ITEMS, 9)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                ItemStack stack = ItemStack.of(entryTag.getCompound(TAG_ITEM));
    ...
    public static void write(ItemStack bag, Tray tray) {
        ...
        for (int slot = 0; slot < SLOTS; slot++) {
            ...
            if (!stack.isEmpty()) {
                ...
                // 1.20.1 的 save(tag) 是原地写入（无返回值）：数量固定写 1，
                // 真实数量记在下面的 Count 里；Count 必须 > 0，否则读回来会被判成空物品。
