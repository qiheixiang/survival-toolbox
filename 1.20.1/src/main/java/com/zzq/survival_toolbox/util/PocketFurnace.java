 ...
 * 次元袋里的熔炉（"熔炉页"的数据与逻辑）
 * <p>
 * <b>规则全部取自原版，不自己编</b>（只做熔炉、调用原版逻辑）：
 * <ul>
 *   <li>配方：原版 {@link RecipeType#SMELTING}，用 {@code RecipeManager} 查表，不写死任何配方；</li>
 *   <li>燃料：原版燃烧时间（1.20.1 走 {@code ForgeHooks.getBurnTime}，就是原版燃料表）；</li>
...
public final class PocketFurnace {
    ...
    /** 产物格（默认产物放这个格子里，和原版熔炉一样；按钮可切成直接进袋子） */
     ...
     * 只读地看一眼自定义数据。
     * <p>
     * 1.20.1 的 {@code ItemStack#getTag()} 返回的就是标签本身的引用（不复制），
     * 所以每 tick 检查一次很便宜：本地模式的袋子 NBT 里装着全部页，复制一次太浪费。
     * </p>
     */
    ...
    public static State read(ItemStack bag) {
        ...
        st.input = readStack(root.getCompound(TAG_INPUT));
        st.fuel = readStack(root.getCompound(TAG_FUEL));
        st.output = readStack(root.getCompound(TAG_OUTPUT));
        ...
        st.cookTarget = readStack(root.getCompound(TAG_COOK_TARGET));
    ...
    public static void write(ItemStack bag, State st) {
        // 复制出来改、最后写回（见 ItemNbt 的线程安全说明）
        CompoundTag root = ItemNbt.copyForEdit(bag);
        root.put(TAG_INPUT, writeStack(st.input));
        root.put(TAG_FUEL, writeStack(st.fuel));
        root.put(TAG_OUTPUT, writeStack(st.output));
        ...
        root.put(TAG_COOK_TARGET, writeStack(st.cookTarget));
    ...
    public static void awardXp(ServerPlayer player, ItemStack bag) {
        State st = read(bag);
        int whole = (int) st.xp;
        if (whole <= 0) return;
    ...
    private static CompoundTag writeStack(ItemStack stack) {
    ...
    public static boolean tick(ServerPlayer player, ItemStack bag) {
        ...
        if (!burning && canSmelt && !st.fuel.isEmpty()) {
            int burnTime = ForgeHooks.getBurnTime(st.fuel, RecipeType.SMELTING);
            if (burnTime > 0) {
        ...
        if (burning && canSmelt) {
            ...
            if (cookProgress(st, now) >= st.cookTotal) {
                if (finishOne(player, bag, st, recipe)) {
    ...
    public static SmeltingRecipe findRecipe(net.minecraft.world.level.Level level, ItemStack input) {
        ...
        // 输入格对齐原版熔炉的三格布局（0 = 输入、1 = 燃料、2 = 产物），
        // 免得有 mod 的熔炼配方去读燃料格时下标越界
        SimpleContainer container = new SimpleContainer(3);
        container.setItem(0, input.copy().split(1));
     ...
     * @return 是否真的完成了
     */
    private static boolean finishOne(ServerPlayer player, ItemStack bag, State st, SmeltingRecipe recipe) {
