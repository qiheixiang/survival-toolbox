 ...
 * 功能页的"原版逻辑引擎"（铁砧 / 锻造台）
 * <p>
 * 铁砧页与锻造台页要<b>留在次元袋界面里</b>（在右侧展开一个小面板），不许像以前那样
 * 直接把玩家丢进原版界面；同时"调用原版逻辑，不自己写"。
 * </p>
 * <p>
 * 做法：在服务端偷偷建一个<b>原版菜单对象</b>（{@link AnvilMenu} / {@link SmithingMenu}）当纯逻辑引擎——
 ...
 *       （扣经验等级、消耗输入、消耗材料、触发 Forge 的 onAnvilRepair 等）；</li>
...
public final class PocketPageEngine {
     ...
     * <p>
     * ⚠️ 原版 {@code AnvilMenu#setItemName} 不接受 null（1.21.1 的 verifyName 会直接 NPE 把游戏崩掉，
     * 1.20.1 也会把 null 存进 itemName 造成后续异常），所以这里必须把 null 转成空串。
     * </p>
     */
