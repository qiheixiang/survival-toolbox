...
public final class ClientHooks {
    ...
    /**
     * 打开原版<b>创造模式物品栏</b>（"创造口袋"右键使用；生存模式也能真正取出物品，见
     * {@code MultiPlayerGameModeMixin} 与 {@code ServerGamePacketListenerImplMixin}）。
     * <p>
     * ⚠️ 只在客户端调用（签名里不出现客户端类型，见类注释）。
     * 界面沿用原版整套结构（搜索框、分类页、垃圾桶），这里只是解除"必须处于创造模式"这一限制，
     * 对"携带创造口袋的玩家"放开；取物由服务端按原版创造模式的规则发放。
     * </p>
     */
    public static void openCreativePicker() {
        ...
        mc.setScreen(new net.minecraft.client.gui.screens.inventory.CreativeModeInventoryScreen(
                mc.player, mc.player.connection.enabledFeatures(), operatorTab));
    }
    ...
    /**
     * 黑白名单同步数据到达客户端后替换手上那份名单物品。
     *
     * @param newStack 服务端同步过来的名单物品
     */
    public static void applyBlacklistSync(ItemStack newStack) {
        Player player = Minecraft.getInstance().player;
