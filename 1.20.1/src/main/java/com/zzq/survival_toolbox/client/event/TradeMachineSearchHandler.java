 ...
 * <p>
 * 交易机的界面就是原版村民界面（菜单类型被写死成原版 {@code MerchantScreen}，
 * 自建界面类不会被实例化），无法往上面加控件，
 * 所以在这些事件里"叠"上去：
 * <ul>
 *   <li>{@code Init.Post} —— 有界面初始化：不是交易机界面就把标记清掉；</li>
 *   <li>{@code ContainerScreenEvent.Render.Foreground} —— 画输入框（底色 / 边框 / 文字 / 光标）。
 *       用它而不是 {@code ScreenEvent.Render.Post}：① 1.21.1 那边容器界面根本不发 Render.Post
 *       （两版本代码保持一致，避免同类问题再次出现）；② Foreground 在 {@code renderLabels} 之后发，
 *       画的框正好压住原版左列那句"交易"标签；</li>
 *   <li>{@code MouseButtonPressed.Pre} —— 点在输入框里才吃掉（否则会连带点到格子）；</li>
 ...
 * （Forge 是在 {@code KeyboardHandler} 里按"Pre → 屏幕 → Post"串起来调的）。
 * </p>
 */
