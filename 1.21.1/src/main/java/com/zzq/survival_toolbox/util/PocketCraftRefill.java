...
/**
 * 合成页的"自动补充"开关（需求：<b>照精妙背包那个自动补充按钮</b>）
 * <p>
 * 判定规则：<b>打开</b>时，玩家从次元袋合成页取走产物之后，刚被原版消耗掉的那几格材料
 * 会自动从储物空间补回九宫格（补到取产物之前的样子）；<b>关闭</b>时什么都不补。
 * </p>
 * <p>
 * <b>⚠️ 只补"袋子里真的有的"</b>：每补一件都从储物空间里扣一件（走界面缓存，见
 * {@link #refill}），补不进去就什么都不放——<b>绝不凭空生成物品</b>。
 * 这条是必须遵守的约束：合成页以前出过"料是复制出来的"（见 {@code PocketJeiTransfer} 里那段根因说明），
...
public final class PocketCraftRefill {
    ...
    public static boolean read(ItemStack bag, HolderLookup.Provider registries) {
    ...
    public static void write(ItemStack bag, boolean on, HolderLookup.Provider registries) {
        ...
        try {
            for (int i = 0; i < grid.getContainerSize() && i < before.size(); i++) {
                ...
                if (!place(grid, i, now, was, put)) {
                    // 放不进去：取出来的这份原样还回储物空间（绝不留在手里，也不能凭空消失）
