 ...
 * 随身次元袋存储辅助（多页版）
 * <p>
 * 存储结构：每页一个"箱子"（54 格，每格一种物品、数量 long 无上限自动合并），页数无限。
 * 数据保存在物品的 {@link DataComponents#CUSTOM_DATA} 组件中：
 * {@code PocketData: {Pages: [{Name: <页名>, Items: [{Item: <物品NBT>, Count: <long>}, ...]}, ...]}}
...
public class PocketStorageHelper {
    ...
    /**
     * ⚠️ <b>临时诊断开关</b>（排查"本地模式：放进去什么，重新打开袋子就什么都没了"）。
     * <p>
     * 打开后会把"开袋/保存/入库/出库"的关键计数打到日志里（带 500ms 节流，磁铁那种每 tick 的不会刷屏）。
     * 定位完把这里改成 {@code false} 就能一次性关掉全部诊断输出。
     * </p>
     */
    public static boolean DIAG = true;

    private static long lastDiagAt = 0L;
    private static int diagSkipped = 0;

    /** 诊断输出（节流版；只在 {@link #DIAG} 打开时有效） */
    public static void diag(String msg) {
        if (!DIAG) return;
        long now = System.currentTimeMillis();
        if (now - lastDiagAt < 500L) {
            diagSkipped++;
            return;
        }
        lastDiagAt = now;
        String tail = diagSkipped > 0 ? ("（+" + diagSkipped + " 条同类省略）") : "";
        diagSkipped = 0;
        com.mojang.logging.LogUtils.getLogger().info("[次元袋·诊断] {}{}", msg, tail);
    }
     ...
     * （要求：<b>打开哪页就匹配哪页、取料也只从那页取</b>）——
     * 否则玩家看着第 1 页却在用第 7 页的材料，他自己根本不知道东西是从哪来的。
    ...
    /** 存储模式标记：true = 共享（末影箱式，数据在服务器存档里按玩家存），false = 本地（数据存在袋子里）。缺省视为共享。 */
    ...
    public record FluidEntry(CompoundTag stackTag, long amount) {
        ...
        /** 是否与给定流体同种（流体 + 组件一致，忽略数量） */
        public boolean sameFluid(net.neoforged.neoforge.fluids.FluidStack other) {
            if (other.isEmpty()) return false;
            net.neoforged.neoforge.fluids.FluidStack mine = toStack(1);
            return !mine.isEmpty() && net.neoforged.neoforge.fluids.FluidStack.isSameFluidSameComponents(mine, other);
        }

        /** 是否与另一条流体条目同种（流体 + 组件一致，忽略数量）：整条搬运合并时用 */
        public boolean sameEntry(FluidEntry other) {
            if (other == null) return false;
            net.neoforged.neoforge.fluids.FluidStack mine = toStack(1);
            net.neoforged.neoforge.fluids.FluidStack theirs = other.toStack(1);
            return !mine.isEmpty() && !theirs.isEmpty()
                    && net.neoforged.neoforge.fluids.FluidStack.isSameFluidSameComponents(mine, theirs);
        }
    }

    /** FluidStack → NBT（数量固定写 1，数量单独记在条目里）；托盘数据层也要用，故为 public */
    public static CompoundTag encodeFluid(net.neoforged.neoforge.fluids.FluidStack stack) {
        net.neoforged.neoforge.fluids.FluidStack copy = stack.copy();
        copy.setAmount(1);
        com.mojang.serialization.DataResult<net.minecraft.nbt.Tag> result =
                net.neoforged.neoforge.fluids.FluidStack.CODEC.encodeStart(net.minecraft.nbt.NbtOps.INSTANCE, copy);
        net.minecraft.nbt.Tag tag = result.result().orElse(null);
        return tag instanceof CompoundTag compound ? compound : new CompoundTag();
    }

    /** NBT → FluidStack（读不出来时返回空） */
    public static net.neoforged.neoforge.fluids.FluidStack decodeFluid(CompoundTag tag) {
        if (tag == null || tag.isEmpty()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        return net.neoforged.neoforge.fluids.FluidStack.CODEC
                .parse(net.minecraft.nbt.NbtOps.INSTANCE, tag)
                .result()
                .orElse(net.neoforged.neoforge.fluids.FluidStack.EMPTY);
    }

    /** 流体页：页名 + 54 个固定流体槽（null = 空槽） */
    ...
    /** 从次元袋物品读取全部页 */
    public static List<Page> readPages(ItemStack bag, HolderLookup.Provider registries) {
        CustomData data = bag.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        if (data.isEmpty()) return new ArrayList<>();
        return readPages(data.copyTag(), registries);
    }

    /**
     * 从一段 NBT 读取全部页（本地模式 = 袋子 CUSTOM_DATA；共享模式 = 服务器存档里该玩家的数据，格式相同）。
     *
     * @param root       含 {@code Pages} 的根标签
     * @param registries 注册表访问
     * @return 页列表（可能为空）
     */
    public static List<Page> readPages(CompoundTag root, HolderLookup.Provider registries) {
        List<Page> pages = new ArrayList<>();
        // 注意：contains 的第二参数为值类型，Pages/Items 均为列表（TAG_LIST=9），若写成 10 将永远判定为 false
        if (root.contains(TAG_PAGES, 9)) {
            ...
            for (int p = 0; p < pagesTag.size(); p++) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    ItemStack stack = ItemStack.parseOptional(registries, entryTag.getCompound("Item"));
    ...
    /** 把全部页写回次元袋物品（只替换 Pages 键，保留同一份自定义 NBT 里的其他键，如存储模式标记） */
    public static void writePages(ItemStack bag, List<Page> pages, HolderLookup.Provider registries) {
        CompoundTag root = ItemNbt.copyForEdit(bag);
        root.put(TAG_PAGES, writePagesTag(pages, registries).getList(TAG_PAGES, 10));
     ...
     */
    public static CompoundTag writePagesTag(List<Page> pages, HolderLookup.Provider registries) {
        ...
        for (int p = 0; p < pages.size(); p++) {
            ...
            for (int slot = 0; slot < page.entries.size(); slot++) {
                ...
                // 1.21.1 的 ItemStack.save(registries, tag) 用 CODEC 编码并以传入 tag 为"前缀"，
                // 实际内容写入并返回的是新 tag，传入的 itemTag 不会被写入。
                // 若丢弃返回值（旧 1.20.1 的原地写入写法），itemTag 只剩下面 put 的 count，
                // 读回时缺 id 解析失败 → 存的物品全部丢失。
                // 必须使用返回值版本：saveOptional 对空物品也安全（返回空 CompoundTag）。
                CompoundTag itemTag = (CompoundTag) e.stack().saveOptional(registries);
    ...
    public static boolean isShared(ItemStack bag) {
        CompoundTag tag = ItemNbt.getTag(bag);
    ...
    private static boolean hasLocalData(CompoundTag tag) {
        CompoundTag root = tag.getCompound(TAG_DATA);
        if (root.contains(TAG_OLD_ITEMS, 9) && !root.getList(TAG_OLD_ITEMS, 10).isEmpty()) return true;
        if (!root.contains(TAG_PAGES, 9)) return false;
        ListTag pages = root.getList(TAG_PAGES, 10);
        for (int p = 0; p < pages.size(); p++) {
    ...
    /** 判断两条物品是否可合并（物品与组件完全相同，忽略数量）。
     * 注意：不能用 ItemStack.isSameItemSameComponents——在 1.21.1 中它会比较 count，
     * 而模板 stack 数量固定为 1，会导致同种物品误判为不同而分成多条。
     * 药水等物品在 NBT 解析时会被 verifyComponentsAfterLoad 规范化（如补写默认组件），
     * 与手持/新放入物品的组件可能不一致导致不合并，因此两边规范化后再比较。 */
    public static boolean sameItem(ItemStack a, ItemStack b) {
        if (!a.is(b.getItem())) return false;
        ItemStack ca = a.copy();
        ItemStack cb = b.copy();
        ca.getItem().verifyComponentsAfterLoad(ca);
        cb.getItem().verifyComponentsAfterLoad(cb);
        return ca.getComponents().equals(cb.getComponents());
    }

    /**
     * 快捷收纳：把物品收进次元袋（手持右键背包物品时调用）。
     * <p>
     * 从第一页开始：有一样的（同物品同组件）就合并进去；没有就找本页空位放入；
     ...
     */
    public static long quickDeposit(ItemStack bag, ItemStack target, HolderLookup.Provider registries) {
        List<Page> pages = readPages(bag, registries);
        // ⚠️ 物品要避开流体格（物品页与流体页同索引共用格子）
        long need = depositInto(pages, readFluidPages(ItemNbt.getTag(bag) == null ? new CompoundTag() : ItemNbt.getTag(bag)), target);
        writePages(bag, pages, registries);
        diag("入库(本地直写) " + target.getItem() + "×" + target.getCount()
                + " 页=" + pages.size() + " 条目=" + totalEntries(pages)
                + " 回读条目=" + totalEntries(readPages(bag, registries))
                + " 袋子实例=" + System.identityHashCode(bag));
        return need;
    }
     ...
     * <p>
     * ⚠️ 不要在其他地方直接 {@code readPages(bag, ...)}：那是"袋子自己的本地页"，
     * 共享模式的袋子读它会永远读到空的（磁铁"只吸袋里有的"就是这样失效的，已修）。
     * </p>
     */
    public static List<Page> readPagesForMode(net.minecraft.world.entity.player.Player player, ItemStack bag,
        ...
        if (isShared(bag) && player != null && !player.level().isClientSide && player.getServer() != null) {
            return PocketSharedStorage.get(player.getServer()).getPages(player.getUUID(), registries);
        }
        return readPages(bag, registries);
    }
     ...
     * <p>
     * 用途：次元袋界面开着时的"匹配/取料"。
     * 判定规则是"<b>打开哪个就匹配哪个</b>"——袋子里第 1 页是木头、第 7 页是木板时，
     * 玩家盯着第 1 页点 JEI 的 +，就不该把第 7 页的木板也算进来（他都看不见那些东西）。
     ...
     */
    public static List<Page> readPagesForMode(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                              int pageIndex, HolderLookup.Provider registries) {
        List<Page> all = readPagesForMode(player, bag, registries);
        if (pageIndex == PAGE_ALL) return all;
        // ⚠️ 这里返回的是"新列表里放着那一页"，不是原列表的 subList：
        //    subList 是视图，调用方一旦 add/remove 就会动到原列表（下面的读取都是只读，但不要留下这个隐患）
    ...
    public static List<FluidPage> readFluidPagesForMode(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                                        HolderLookup.Provider registries) {
        if (isShared(bag) && player != null && !player.level().isClientSide && player.getServer() != null) {
            return PocketSharedStorage.get(player.getServer()).getFluidPages(player.getUUID(), registries);
        }
        return readFluidPages(bag, registries);
    }
     ...
     * 一旦哪条路径没写进去，表现就是"箱子里的东西取走了、袋子里两边都看不到"（实测会丢东西）。
     * </p>
     *
     * @return 该物品在存储里的总数量（同物品同组件，跨页累加）
     */
    public static long countInStorage(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                      ItemStack template, HolderLookup.Provider registries) {
        if (template == null || template.isEmpty()) return 0L;
        long total = 0L;
        for (Page page : readPagesForMode(player, bag, registries)) {
     ...
     * 否则就是"取出来了但其实没落盘"——那意味着凭空多出来一份（历史问题："料是复制出来的"）。
     * </p>
     *
     * @param pageIndex 页下标（0 起）；{@value #PAGE_ALL} = 所有页
     */
    public static long countInStorage(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                      ItemStack template, int pageIndex, HolderLookup.Provider registries) {
        if (template == null || template.isEmpty()) return 0L;
        long total = 0L;
        for (Page page : readPagesForMode(player, bag, pageIndex, registries)) {
            for (Entry e : page.entries) {
                if (e != null && sameItem(e.stack(), template)) total += e.count();
            }
        }
    ...
    public static long fluidAmountInStorage(net.minecraft.world.entity.player.Player player, ItemStack bag,
        ...
        long total = 0L;
        for (FluidPage page : readFluidPagesForMode(player, bag, registries)) {
     ...
     * @return 剩余数量（成功收纳后恒为 0）
     */
    public static long quickDeposit(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                    ItemStack target, HolderLookup.Provider registries) {
        // ⚠️⚠️ 界面开着时**绝不能**直接写存储：界面里的 {@code PocketDimensionContainer} 有一份页缓存，
        //       它每次 save() 都会把缓存整份写回去 —— 绕过缓存写入的东西**当场看不出来、关界面就没了**
        //       （"磁铁吸了东西，开袋子什么都没有"就属于这一类）。
        ...
        if (isShared(bag) && player != null && !player.level().isClientSide && player.getServer() != null) {
            ...
            List<Page> pages = shared.getPages(player.getUUID(), registries);
            // ⚠️ 物品要避开流体格：把流体页一起读出来传给入库逻辑
            long need = depositInto(pages, shared.getFluidPages(player.getUUID(), registries), target);
            shared.setPages(player.getUUID(), pages, registries);
            return need;
        }
        return quickDeposit(bag, target, registries);
    }

    /**
     * 快捷收纳流体（按袋子当前模式写入对应存储）。
     * <p>
     * 与 {@link #quickDeposit(net.minecraft.world.entity.player.Player, ItemStack, ItemStack, HolderLookup.Provider)}
     * 完全同构：共享模式的袋子写服务器存档（按玩家），本地模式写袋子自己的 NBT。
     * 托盘"纳入"、光标流体放回等入口都必须走这里，才能严格跟随当前的共享/本地模式。
     * </p>
     * <p>
     * 注意：物品页与流体页是<b>同一套页索引</b>（界面只有一套页列表、一个混排网格），
     * 所以这里写流体页的同时也可能扩出空的物品页（见
     * {@link #depositFluidEntryInto(List, List, FluidEntry)}），两个键都要写回去。
     * </p>
     *
     * @param player     操作者（判定玩家与服务器；仅服务端有效）
     * @param bag        次元袋
    ...
    public static long quickDepositFluid(net.minecraft.world.entity.player.Player player, ItemStack bag,
        ...
        if (isShared(bag) && player != null && !player.level().isClientSide && player.getServer() != null) {
            PocketSharedStorage shared = PocketSharedStorage.get(player.getServer());
            List<Page> pages = shared.getPages(player.getUUID(), registries);
            List<FluidPage> fluidPages = shared.getFluidPages(player.getUUID(), registries);
            long need = depositFluidInto(fluidPages, pages, stack);
            // 两个键都要写（新页是"物品页 + 流体页"一起扩出来的）
            shared.setPages(player.getUUID(), pages, registries);
            shared.setFluidPages(player.getUUID(), fluidPages, registries);
            return need;
        }
        // 本地：物品页与流体页都在袋子 NBT 里（物品页用来避开被物品占用的格子，也可能被扩出新页）
        CompoundTag root = ItemNbt.getTag(bag);
        List<Page> pages = root == null ? new ArrayList<>() : readPages(root, registries);
     ...
     * 更糟的是"整理"会把它当成没被收集到而**直接清掉**（实测丢过一格水源）。
    ...
    // ============================================================
    // 流体页读写（与物品页同一个根标签，键不同：FluidPages）
    // ============================================================

    /** 从次元袋物品读取全部流体页 */
    public static List<FluidPage> readFluidPages(ItemStack bag, HolderLookup.Provider registries) {
        CompoundTag tag = ItemNbt.getTag(bag);
    ...
    /** 把流体页写回次元袋物品（只替换 FluidPages 键，保留同一份自定义 NBT 里的其他键） */
     ...
     * 收纳纯逻辑：把流体灌进给定流体页列表。
     * <p>
     * 与物品一致：同种流体先合并到已有的格；没有就找本页空位；所有页都满就新建一页。
     * 数量用 long，等于单种流体容量无上限。
     * </p>
     *
     * @param pages 流体页列表（会被就地修改）
     * @param stack 待收纳流体（数量会被全部收下，返回 0）
     * @return 剩余数量（成功收纳后恒为 0）
     */
    public static long depositFluidInto(List<FluidPage> pages, List<Page> itemPages,
                                        net.neoforged.neoforge.fluids.FluidStack stack) {
        if (stack.isEmpty()) return 0;
        return depositFluidEntryInto(pages, itemPages,
                new FluidEntry(encodeFluid(stack), stack.getAmount()));
    }

    /**
     * 收纳纯逻辑（long 数量版）：把一条流体条目并进流体页列表。
     * <p>
     * 与 {@link #depositFluidInto(List, List, net.neoforged.neoforge.fluids.FluidStack)} 同一套规则，
     * 但数量用条目的 long——把本地存储整体并进共享空间这类"整条搬运"必须走这里，
     * 否则超过 int 上限的 mB 会被截断。
     * </p>
     *
     * @param pages     流体页列表（会被就地修改）
     * @param itemPages 物品页列表（只读：用来避开被物品占用的格子）
    ...
    public static long depositFluidEntryInto(List<FluidPage> pages, List<Page> itemPages, FluidEntry entry) {
        ...
        for (int p = 0; p < pages.size(); p++) {
            FluidPage page = pages.get(p);
            // 物品与流体共用同一页的 54 格：被物品占着的格子不能再放流体
    ...
    public static ItemStack withdrawFromStorage(net.minecraft.world.entity.player.Player player, ItemStack bag,
                                                ...
                                                HolderLookup.Provider registries) {
        return withdrawFromStorage(player, bag, template, count, PAGE_ALL, registries);
    }

    /**
     * 从袋子里取出指定物品（<b>跨页或只取某一页</b>；严格按当前共享/本地模式，仅服务端）。
     * <p>
     * ⚠️⚠️ <b>界面开着时必须走界面那份页缓存</b>（和 {@code quickDeposit} 同一个问题，已补上）：
    ...
    public static ItemStack withdrawFromStorage(net.minecraft.world.entity.player.Player player, ItemStack bag,
        ...
        List<Page> pages;
        if (shared) {
            pages = store.getPages(player.getUUID(), registries);
        } else {
            CompoundTag root = ItemNbt.getTag(bag);
            pages = root == null ? new ArrayList<>() : readPages(root, registries);
        }
        ...
        if (shared) {
            // 只写物品页（流体页没动）
            store.setPages(player.getUUID(), pages, registries);
        } else {
            writePages(bag, pages, registries);
            // 诊断：出库也回读一次（JEI 的"+"号是从这里取料的，曾出现过"料是复制出来的"）
            diag("出库(本地) " + template.getItem() + "×" + taken
                    + " 页=" + pages.size() + " 条目=" + totalEntries(pages)
                    + " → 回读条目=" + totalEntries(readPages(bag, registries))
                    + " 袋子实例=" + System.identityHashCode(bag));
        }
     ...
     * 看起来像把水存成了 1000 个桶（显示上会让人以为存的是水桶而不是流体）。
     * </p>
     * <p>
     * 为什么要把流体本身也塞进去：很多模组流体（机械动力那些）**根本没有桶物品**，
     * 图标只能退化成空桶，界面就没法显示"这是什么流体"了。带上流体本体后，
     * 客户端能拿到 {@link FluidStack} 去画它的静止贴图、也才能给出正确的 tooltip。
     * 这个栈是**临时显示用**的，会随槽位/同步包原样到客户端（不要拿去当真实物品用）。
     * </p>
     */
     ...
     * 原版的数量是它自己画的，另外叠一个缩写上去只会得到"两个数字"或者"缩写被盖住" ——
     * 曾出现过该问题（实测：19456 原样显示，缩写完全没有出现）。
     * </p>
     */
    ...
    public static net.neoforged.neoforge.fluids.FluidStack displayFluid(ItemStack stack) {
        if (stack == null || stack.isEmpty()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        net.minecraft.nbt.CompoundTag tag = ItemNbt.getTag(stack);
     ...
     * </p>
     */
    public static boolean hasBucket(net.neoforged.neoforge.fluids.FluidStack fluid) {
