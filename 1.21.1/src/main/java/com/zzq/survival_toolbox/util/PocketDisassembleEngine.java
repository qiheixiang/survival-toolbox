 ...
 * <p>
 * 需求：拆解台也要像铁砧 / 锻造台 / 合成台那样<b>在次元袋界面里展开</b>，
 * 而且"逻辑去引用原本的，不要自己写"。所以这里照 {@link PocketPageEngine} 的老套路：
 * 在服务端构造一个不对外开放的<b>拆解台菜单对象</b>当纯逻辑引擎——它不注册给玩家
...
public final class PocketDisassembleEngine {
     ...
     * 这是"拆一部分材料"的收费规则。直接改容器会绕过收费 = 未消耗即可拿到材料。
     * </p>
     */
     ...
     * 没成的话调用方必须用 {@link #putOutputBack(ItemStack)} 把产物放回去，绝不能让玩家未消耗即获得产物。
     */
