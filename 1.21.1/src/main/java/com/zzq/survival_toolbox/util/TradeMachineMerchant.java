...
/**
 * 交易机的"随身村民"：把交易机物品里记录的报价包成原版 {@link Merchant}，
...
public class TradeMachineMerchant implements Merchant {
    ...
    public TradeMachineMerchant(Player player, ItemStack machine) {
        ...
        this.source = new ArrayList<>(TradeMachineData.read(machine, player.level().registryAccess()));
    ...
    private void buildOffers() {
        ...
        for (TradeMachineData.Trade t : this.source) {
            ...
            this.offers.add(new MerchantOffer(new net.minecraft.world.item.trading.ItemCost(t.costA.getItem(), Math.max(1, t.costA.getCount())), t.costB.isEmpty() ? java.util.Optional.<net.minecraft.world.item.trading.ItemCost>empty() : java.util.Optional.of(new net.minecraft.world.item.trading.ItemCost(t.costB.getItem(), Math.max(1, t.costB.getCount()))), t.result.copy(), Math.max(0, t.uses), Integer.MAX_VALUE, 0, 0.0F));
    ...
    public void notifyTrade(MerchantOffer offer) {
        ...
        TradeMachineData.write(this.machine, this.source, this.player.level().registryAccess());
    }
