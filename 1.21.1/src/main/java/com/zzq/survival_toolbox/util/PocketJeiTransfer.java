 ...
 * </p>
 * <p>
 * <b>⚠️⚠️ 为什么每一步都要"回读确认 + 回滚"</b>（实测过的物品复制缺陷，请勿简化掉）：
 * 老写法是"先往格子里摆一份、再从袋子/背包扣一份"，两件事<b>各自独立</b>，
 * 任何一步没生效（界面开着时绕过页缓存扣减会被缓存整份盖回来、原版槽位不接受这个物品……）
 * 的结果都是<b>格子里的材料未消耗即多出一份</b>，而且玩家能把它再放回袋子 = 无限刷物品
 * （故障表现：物品不消耗，放进去的不是袋子里的，而是额外复制出来的，还能放回袋子）。
...
public final class PocketJeiTransfer {
     ...
     * <p>
     * 约定：袋子界面下点 JEI 的 +，<b>哪个面板开着就摆进哪个面板的九宫格</b>；
     * 工作台页和拆解页都没开就什么都不摆（老代码会在这里自动打开工作台页，
     * 表现是「没开面板也照样匹配，匹配的还是工作台」）。
     * </p>
     * <p>
     ...
     *       而且每 tick 都会被引擎重算并覆盖。<b>这时候绝不能碰它</b>：摆了会被下一 tick 覆盖掉（= 白白消耗玩家的材料），
    ...
    public static void apply(ServerPlayer player, List<List<ItemStack>> plan, int inputSlotStart, int inputSlotCount,
        ...
        for (int k = 0; k < inputSlotCount; k++) {
            ...
            //    否则"还给玩家的"和"还留在格子里的"是同一件 → 又白来一份
            if (!slot.getItem().isEmpty()) continue;
            returnToOwner(player, bag, back, registries);
        }
        ...
        for (int k = 0; k < plan.size() && k < inputSlotCount; k++) {
            ...
            ItemStack one = takeOne(player, bag, candidates, page, registries);
            if (one.isEmpty()) {
            ...
            // 那样这一份就得还回去，否则就是"未消耗即多出一份"（历史物品复制缺陷）
            if (!matchesAny(candidates, slot.getItem())) {
                returnToOwner(player, bag, one, registries);
                missing.add(firstNonEmpty(candidates));
            }
        }
    ...
    public static boolean canFill(ServerPlayer player, List<List<ItemStack>> plan, int inputSlotStart,
        ...
        //    让 + 号直接点不动，而不是点了没反应（客户端那边还会给一句"先打开工作台页/拆解页"）。
        if (menu instanceof PocketDimensionMenu pocket
                && resolveInputSlotStart(pocket) != inputSlotStart) {
            return false;
        }
        ...
        if (!bag.isEmpty()) {
            try {
                for (PocketStorageHelper.Page p
                        : PocketStorageHelper.readPagesForMode(player, bag, page, registries)) {
    ...
    private static ItemStack takeOne(ServerPlayer player, ItemStack bag, List<ItemStack> candidates, int pageIndex,
        ...
        for (ItemStack want : candidates) {
            ...
            for (int i = 0; i < inv.items.size(); i++) {
                ...
                // 这一格空了就清掉（拿走的是一份"活堆叠"，上面已经确认过就是这一格）
        ...
        for (ItemStack want : candidates) {
            ...
            long beforeCount = PocketStorageHelper.countInStorage(player, bag, want, pageIndex, registries);
            if (beforeCount <= 0L) continue;
            ItemStack got = PocketStorageHelper.withdrawFromStorage(player, bag, want, 1, pageIndex, registries);
            if (got.isEmpty()) continue;
            long afterCount = PocketStorageHelper.countInStorage(player, bag, want, pageIndex, registries);
            if (afterCount > beforeCount - 1L) {
                // 扣减没落盘（会被界面缓存整份盖回来）：这一份不能算数，
                // 原样放回去（放回背包；背包也放不下就掉在脚下，不允许凭空多出物品）
                returnToOwner(player, bag, got, registries);
                continue;
            }
            got.setCount(1);
            return got;
    ...
    private static void returnToOwner(ServerPlayer player, ItemStack bag, ItemStack stack,
        ...
        if (!bag.isEmpty()) {
            long left = PocketStorageHelper.quickDeposit(player, bag, copy, registries);
            if (left <= 0) return;
