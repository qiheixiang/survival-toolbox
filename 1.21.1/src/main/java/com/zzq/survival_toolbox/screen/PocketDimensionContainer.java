...
public class PocketDimensionContainer implements Container {
    ...
    public PocketDimensionContainer(ItemStack bag, Player owner, HolderLookup.Provider registries) {
        this.bag = bag;
        this.owner = owner;
        ...
        if (owner == null || owner.level().isClientSide) {
            ...
            // 诊断：开袋这一刻到底读到了什么（排查"重开就没了"，见 PocketStorageHelper#DIAG）
            PocketStorageHelper.diag("开袋 模式=" + (shared ? "共享" : "本地")
                    + " 页=" + pages.size() + " 条目=" + PocketStorageHelper.totalEntries(pages)
                    + " 袋NBT直读条目=" + PocketStorageHelper.totalEntries(PocketStorageHelper.readPages(bag, registries))
     ...
     * 那片流体是**隐形**的，而且只要整理一次就会被清掉（实测已丢失过一格水源）。
     * 现在开袋时就把这种流体系目挪到第一个"物品页与流体页都空着"的格子上，同时把存档修复干净。
     * </p>
     */
    ...
    private List<PocketStorageHelper.Page> loadPagesFromSource() {
        if (shared && owner != null && owner.getServer() != null) {
            return PocketSharedStorage.get(owner.getServer()).getPages(owner.getUUID(), registries);
        }
        return PocketStorageHelper.readPages(bag, registries);
    }

    /** 按当前模式从对应存储读取流体页（仅服务端） */
    private List<PocketStorageHelper.FluidPage> loadFluidPagesFromSource() {
        if (shared && owner != null && owner.getServer() != null) {
            return PocketSharedStorage.get(owner.getServer()).getFluidPages(owner.getUUID(), registries);
        }
        return PocketStorageHelper.readFluidPages(bag, registries);
    }
     ...
     * 往当前存储里灌流体（服务端）。
     * <p>
     * 与物品收纳一致：先按流体种类合并，再找空位，所有页满就新建一页；数量 long 无上限，
     * 因此永远收得下，返回 0。
     * </p>
     *
     * @param stack 待灌入的流体
     * @return 剩余数量（恒为 0）
     */
    public long insertFluid(net.neoforged.neoforge.fluids.FluidStack stack) {
    ...
    public boolean sort(int scope, int sortBy) {
        ...
        //     结果就是"整理一次，被物品压着的那格水源凭空消失"（实测已丢失过一格水源）。
        //     两个都收集之后，排序会把它们摆到不同的格子上，同时就把这种脏数据修复了。
        ...
        for (SortItem it : merged) {
            ...
            // 必须遵守的约束：物品页与流体页共用页索引，需要新页时两边一起建
     ...
     * 这件东西的"标签键"（按标签排序用）。
     * <p>
     * 一个物品常常带一堆标签，直接取第一个会把木头排到 `minecraft:mineable/axe` 里去 ——
    ...
    /**
     * 看一眼某个显示格上的流体（不取出），用于判断能不能灌进容器。
     *
     * @param slot 视图内槽位（会加上翻页偏移）
     * @return 该格的流体模板（数量为 1）；空 = 该格没有流体
     */
    public net.neoforged.neoforge.fluids.FluidStack peekFluidAt(int slot) {
        if (clientSide) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        int idx = pageOffset + slot;
        if (idx < 0 || idx >= displayRefs.size()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        EntryRef ref = displayRefs.get(idx);
        if (ref.page < 0 || ref.page >= fluidPages.size()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        PocketStorageHelper.FluidEntry entry = fluidPages.get(ref.page).entries.get(ref.slot);
        return entry == null ? net.neoforged.neoforge.fluids.FluidStack.EMPTY : entry.toStack(1);
    }
     ...
     *
     * @param slot      视图内槽位（会加上翻页偏移）
     * @param maxAmount 最多取多少 mB
     * @return 实际取出的流体（空 = 该格没有流体）
     */
    public net.neoforged.neoforge.fluids.FluidStack extractFluidAt(int slot, long maxAmount) {
        if (clientSide) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        int idx = pageOffset + slot;
        if (idx < 0 || idx >= displayRefs.size()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        EntryRef ref = displayRefs.get(idx);
        if (ref.page < 0 || ref.page >= fluidPages.size()) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        PocketStorageHelper.FluidPage page = fluidPages.get(ref.page);
        PocketStorageHelper.FluidEntry entry = page.entries.get(ref.slot);
        if (entry == null || entry.amount() <= 0) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        long take = Math.min(maxAmount, entry.amount());
        if (take <= 0) return net.neoforged.neoforge.fluids.FluidStack.EMPTY;
        net.neoforged.neoforge.fluids.FluidStack out = entry.toStack(take);
        if (out.isEmpty()) return out;
        long left = entry.amount() - take;
        page.entries.set(ref.slot, left <= 0 ? null : new PocketStorageHelper.FluidEntry(entry.stackTag(), left));
        rebuildDisplay();
        save();
        return out;
    }
     ...
     * <p>
     * 需求：老存档可能把一大堆东西存在本地，因此需要一个一键转移；<b>只做本地 → 共享</b>；
    ...
    public MergeResult mergeLocalToShared() {
        ...
        List<PocketStorageHelper.Page> localPages = PocketStorageHelper.readPages(bag, registries);
        List<PocketStorageHelper.FluidPage> localFluids = PocketStorageHelper.readFluidPages(bag, registries);
        long itemCount = 0L;
        long fluidAmount = 0L;
        ...
        List<PocketStorageHelper.Page> sharedPages = store.getPages(owner.getUUID(), registries);
        List<PocketStorageHelper.FluidPage> sharedFluids = store.getFluidPages(owner.getUUID(), registries);
        if (isStorageEmpty(sharedPages, sharedFluids)) {
            // 共享还是空的：整份搬过去，页名原样保留
            store.setPages(owner.getUUID(), localPages.isEmpty() ? sharedPages : localPages, registries);
            store.setFluidPages(owner.getUUID(), localFluids.isEmpty() ? sharedFluids : localFluids, registries);
        } else {
            // 共享已有内容：逐条"加"上去，绝不覆盖
            for (PocketStorageHelper.Page p : localPages) {
                for (PocketStorageHelper.Entry e : p.entries) {
            ...
            store.setPages(owner.getUUID(), sharedPages, registries);
            store.setFluidPages(owner.getUUID(), sharedFluids, registries);
        }
     ...
     * （老写法只取一组却把整格覆盖掉，袋子里动辄几千的大堆叠会被吃掉。）
     * </p>
     */
     ...
     */
    public long insertFluidAt(int slot, net.neoforged.neoforge.fluids.FluidStack stack) {
...
, fa);
    ...
    private static boolean fluidMatches(PocketStorageHelper.FluidEntry entry, String keyword) {
        ...
        net.neoforged.neoforge.fluids.FluidStack stack = entry.toStack(1);
    ...
    /** 页名列表（服务端 = 真实页名，按当前视图取物品页或流体页；客户端 = 同步的页名） */
    ...
    /** 每页条目数（服务端 = 真实计数，按当前视图；客户端 = 同步的计数） */
    ...
    public void save() {
        ...
        if (shared && owner != null && owner.getServer() != null) {
            PocketSharedStorage store = PocketSharedStorage.get(owner.getServer());
            store.setPages(owner.getUUID(), pages, registries);
            store.setFluidPages(owner.getUUID(), fluidPages, registries);
            PocketStorageHelper.diag("保存(共享) 页=" + pages.size() + " 条目="
                    + PocketStorageHelper.totalEntries(pages));
        } else {
            PocketStorageHelper.writePages(bag, pages, registries);
            PocketStorageHelper.writeFluidPages(bag, fluidPages);
            // 诊断：写完立刻从物品 NBT 直读一次，看看到底落没落盘（排查"重开就没了"）
            PocketStorageHelper.diag("保存(本地) 页=" + pages.size() + " 条目="
                    + PocketStorageHelper.totalEntries(pages)
                    + " → 袋NBT直读条目=" + PocketStorageHelper.totalEntries(PocketStorageHelper.readPages(bag, registries))
                    + " 袋子实例=" + System.identityHashCode(bag));
        }
     ...
     * 下一次 {@link #save()} 用旧缓存整份覆盖掉 —— 表现就是"东西放进去就没了"（实测出现过）。
    ...
    public long quickDepositCached(ItemStack stack) {
        ...
        // ⚠️ 把 fluidPages 一起传进去：物品不能落在"被流体占着"的格子上（否则那片流体隐形、整理时还会被清掉）
     ...
     * <p>
     * ⚠️ 与 {@link #quickDepositCached} 是同一个问题的两面：界面开着的时候，页数据在本对象里有一份缓存，
     * 任何"绕过缓存直接改袋子 NBT / 共享存档"的扣减都会被下一次 {@link #save()} 用旧缓存<b>整份盖回来</b>
     * ——表现就是"料取出来了、袋子里一个没少"，也就是 JEI 的 + <b>凭空复制材料</b>（历史问题）。
     * 所以取料也必须走这里。
     * </p>
     * <p>
     * 判定规则：<b>打开哪页就只从哪页取</b>（玩家看得见的东西才算数）。
