 ...
 *       每次点击前都会先刷新（见菜单），否则"服务端每 tick 改 NBT"与"玩家点击改缓存"会打架，
 *       出现把烧掉的输入又写回去这种复制物品的问题。</li>
 *   <li>客户端：格子内容由原版槽位同步写入，进度（火焰/箭头）由次元袋同步包写入，只用于显示。</li>
 * </ul>
 * 页里没有输出槽：产物烧好直接按当前共享/本地模式收进袋子存储（设计目标是"产物回仓库"）。
 * </p>
 */
public class PocketFurnaceContainer implements Container {
    ...
    public PocketFurnaceContainer(ItemStack bag, HolderLookup.Provider registries,
                                  net.minecraft.world.level.Level level, boolean clientSide) {
        this.bag = bag;
    ...
    public void refreshFromBag() {
        ...
        PocketFurnace.State st = PocketFurnace.read(bag, registries);
    ...
    public void save() {
        ...
        PocketFurnace.write(bag, st, registries);
    }
