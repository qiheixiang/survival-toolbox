...
public class SurvivalToolboxJeiPlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.fromNamespaceAndPath(SurvivalToolbox.MODID, "jei_plugin");
    }
     ...
     * 本模组为了不把 JEI 整个挤掉，面板展开时<b>不改 imageWidth</b>（面板直接画在界面外面），
    ...
    public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration) {
        ...
        // 原版锻造台（本模组用按钮打开的那个界面）：材料可以先从袋子所有页取到背包，再交给 JEI 放进输入槽。
     ...
     * 这里在真正转移之前点一下菜单按钮 4（走原版容器按钮包，不需要自定义网络包），
