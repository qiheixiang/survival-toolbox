 ...
 * 从袋子取出、整理…），客户端不可能知道；靠 TTL 兜住这种"客户端没看见的变化"，
 * 过期就重新问一次。TTL 太长会拿着过期的"凑得齐"让人点了没反应，太短会一直发包刷服务器。
 * </p>
 * <p>
 * <b>"还没答案"也占一个状态位（PENDING）</b>：问出去之后到答案回来之前这段时间，
 ...
 * 线程：全部读写都在客户端主线程（JEI 判定 + 包处理都走 enqueueWork），不需要加锁。
 * </p>
 */
public final class PocketJeiAvailabilityCache {
    ...
    /** 缓存最多留多少条（防止长时间游玩后无限攒；超了先清最老的几条） */
    ...
    /** 两次查询之间至少隔这么久（毫秒）：袋子身份一变签名就变，别让它在极端情况下把包刷爆 */
    ...
    /**
     * 查一次：没问过或已过期就返回 {@link #UNKNOWN}，调用方负责记下"在问了"并真的把包发出去。
     * <p>
     * ⚠️ 返回 {@link #UNKNOWN} 时调用方<b>不能</b>当成"可以点"（那正是这个 bug 的根）。
     * </p>
     */
    ...
    /** 条数太多就整体清一遍（这东西只是加速用的，清掉最多多问几次服务端） */
