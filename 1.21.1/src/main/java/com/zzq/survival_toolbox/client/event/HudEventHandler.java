...
public class HudEventHandler {
    ...
    private static float[] getTotalShield(Player player) {
        ...
        for (ItemStack armor : player.getArmorSlots()) {
            if (armor.getEnchantments().getLevel(ModEnchantments.adaptation(player.level().registryAccess())) > 0) {
                totalCurrent += ItemNbt.getOrCreateTag(armor).getFloat("adapt_shield_current");
                totalMax += ItemNbt.getOrCreateTag(armor).getFloat("adapt_shield_max");
            }
        }
